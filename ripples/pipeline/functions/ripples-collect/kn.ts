...
export async function kfetch(b: Budget, url: string, init: RequestInit = {}, opts: { aqs?: boolean; timeoutMs?: number } = {}): Promise<Response> {
  ...
  const api = u.pathname.startsWith("/w/api.php");
  const key = `${u.hostname}${api ? " api" : ""}`;
  for (let attempt = 0; attempt < 2; attempt++) {
    await pace(key, api ? SPACING.api : u.hostname === "wikimedia.org" ? SPACING.aqs : SPACING.other);
...
export async function pool<T, R>(items: T[], conc: number, fn: (x: T) => Promise<R>): Promise<(R | Error)[]> {
  ...
  if (halt) { (halt as any).partial = out; throw halt; }
  return out;
}

// Results completed before a Stop/OutOfBudget aborted a pool (the halting error carries them).
export function partialOf<R>(e: unknown): R[] {
  const p = (e as any)?.partial;
  return Array.isArray(p) ? p.filter((x) => x && !(x instanceof Error)) as R[] : [];
}
