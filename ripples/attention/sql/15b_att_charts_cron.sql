...
-- / 06:25 SPEC resolve stage. Apple needs several slots: 25 feeds at the 5 s floor (DEMARCATION Q6, end-to-start) plus
-- Apple's ~8 s response time give ~7 feeds per 110 s run; the mode is resumable (att_state 'charts.apple') and a slot
-- with nothing left makes no request. (Applied first with three slots as migration att_charts_cron, then replaced by
-- six slots in att_charts_cron2.)
...
declare j text;
begin
  foreach j in array array['att-apple-1','att-apple-2','att-apple-3','att-apple-4','att-apple-5','att-apple-6','att-steamspy','att-hf','att-github',
...
select cron.schedule('att-apple-2',     '12 6 * * *', $$select public.call_collector('att-charts', '{"mode":"apple"}'::jsonb)$$);
select cron.schedule('att-apple-3',     '14 6 * * *', $$select public.call_collector('att-charts', '{"mode":"apple"}'::jsonb)$$);
select cron.schedule('att-apple-4',     '16 6 * * *', $$select public.call_collector('att-charts', '{"mode":"apple"}'::jsonb)$$);
select cron.schedule('att-apple-5',     '18 6 * * *', $$select public.call_collector('att-charts', '{"mode":"apple"}'::jsonb)$$);
select cron.schedule('att-apple-6',     '20 6 * * *', $$select public.call_collector('att-charts', '{"mode":"apple"}'::jsonb)$$);
