...
function missing(n) {
  ...
  put($('seed'), h('p', { class: 'eb' }, 'Not here yet'),
    h('h2', null, n ? `Knock-On ${n < 0 ? 'practice ' + n : '#' + n} isn't available` : "Couldn't load today's puzzle"),
    h('p', { class: 'sub' }, n ? 'It may not be live yet. Puzzles go live at 07:30 UTC.' : 'Check your connection and try again.'),
...
function prelaunch(l) {
  ...
  $('seed').removeAttribute('aria-busy');
  put($('seed'), h('p', { class: 'eb' }, 'Starts soon'),
...
// ---------- seed ----------
const SRC = { gtrends: '🔎', bsky: '🦋', mastodon: '🐘', gdelt_tv: '📺', autocomplete: '⌨️', polymarket: '📊', wiki: '📖' };
...
function crossRow(items) {
  ...
  return [h('p', { class: 'xlab' }, 'Also on other channels · corroboration only, not part of the measurement'),
    h('ul', { class: 'bdgs' }, items.map(x => h('li', { class: 'bdg x' },
      `${SRC[x.source] || '•'} ${x.label}${x.multiple != null ? ' · ' + fmtM(x.multiple) : ''}${x.when ? ' · ' + x.when : ''}`)))];
}
...
function renderSeed() {
  ...
  put(el, 
    h('p', { class: 'eb' }, `${P.date === L.currentPuzzleDate() ? "Today's trend" : 'Trend of ' + fmtD(P.date)} · ${L.CAT_NAMES[sd.category] || ''}`),
    h('h2', null, h('span', { class: 'em', 'aria-hidden': 'true' }, sd.emoji), sd.title),
    P.headline && P.headline.text ? h('p', { class: 'headline' }, P.headline.text, P.headline.source === 'ai' ? [' ', h('span', { class: 'tag' }, 'AI-written')] : null) : null,
    h('p', { class: 'hl' }, L.fmtBiggestIn(sd)),
    h('p', { class: 'sub' }, `${fmtM(sd.multiple)} its normal Wikipedia readers${sd.langs > 1 ? ` · ${sd.langs} languages` : ''}`),
    ...
    h('p', { class: 'cap' }, `Daily Wikipedia readers, 90 days to ${fmtD(P.data_date)}. Shaded: baseline ${fmtD(bl.from)} – ${fmtD(bl.to)}, median ${L.fmtInt(bl.median)}/day (dashed).`),
...
function renderRound(k) {
  ...
  const opts = h('div', { class: 'opts', role: 'group', 'aria-label': 'Four linked pages' },
    r.options.map(o => h('button', { type: 'button', class: 'opt', 'data-id': o.id, onclick: e => pick(k, o.id, e.currentTarget, say, rv) },
  ...
  put($('round'), h('div', { class: 'card' },
    ...
    h('p', { class: 'eb' }, `Round ${k + 1} of ${R()}`),
    !c.continues ? h('p', { class: 'fresh' }, '🌊 ', h('b', null, 'New ripple from today’s seeds: '), `${r.seed.emoji} ${r.seed.title}`,
    ...
    prompt,
    !k && !Object.keys(S.hist).length ? h('p', { class: 'coach' }, 'Pick one. You get two tries.') : null,
    opts, say, rv));
  if (k) { prompt.focus({ preventScroll: true }); $('round').scrollIntoView({ behavior: RM ? 'auto' : 'smooth', block: 'start' }); }
}
...
function revealRound(k, rv) {
  ...
  put(grid, ...r.options.map(o => {
    ...
    return h('div', { class: cls },
      ...
      h('span', { class: 'rl' }, isA ? [h('strong', null, `${fmtM(m.multiple)} normal`), ` · ${L.fmtTiming(m.lag_days, m.timing)}`] : `also linked; views normal, ${fmtM(m.multiple)}`),
      srow && srow.split ? h('span', { class: 'pp' }, `players' picks ${srow.split[o.id] ?? 0}%`) : null,
  ...
  const big = h('span', { class: 'big' }, '1.0×');
  ...
  put(rv, h('div', { class: 'rvl pop' },
    ...
    h('p', { class: 'fl' }, h('i', { style: `--f:${fl}%`, 'aria-hidden': 'true' }), `🎲 ${L.fmtFluke(ev)}${ev.fluke_warming ? ' (this hop also passed a stricter time-shift test)' : ''}`),
    h('ul', { class: 'bdgs' }, bdg.map(b => h('li', { class: 'bdg' }, b))),
    crossRow(ev.cross),
    srow ? h('p', { class: 'meas' }, `Players' picks: ${srow.first_try_pct}% found it first try, ${srow.found_pct}% found it at all.`) : null,
    cap ? h('p', { class: 'quote' }, cap.text, cap.source === 'ai' ? h('span', { class: 'tag' }, 'AI-written context') : cap.source === 'wikipedia' ? h('span', { class: 'tag' }, 'Wikipedia description') : null) : null,
    rr.shared_trigger_stop ? h('p', { class: 'stop' }, '⛔ The chain stops here: the next page may be reacting to the same news as the seed, so it is not counted as a knock-on.') : null,
    ...
    k === R() - 1 && S.rv.end ? h('p', { class: 'stop' }, S.rv.end.text) : null), nextBtn(k));
  countUp(big, a.multiple, fmtM);
  rv.scrollIntoView({ behavior: RM ? 'auto' : 'smooth', block: 'nearest' });
}
function nextBtn(k) {
  const last = k === R() - 1;
  return h('div', { class: 'nextrow' }, h('button', { type: 'button', class: 'btn wide', onclick: () => (last ? renderFinal() : renderRound(k + 1)) }, last ? 'Last question →' : 'Next round →'));
}
...
function renderFinal() {
  ...
  put($('round'), h('div', { class: 'card' }, trail(R()), h('p', { class: 'stop' }, (S.rv && S.rv.end && S.rv.end.text) || 'The measured trail ends here.')));
  const read = h('p', { class: 'read unset', 'aria-hidden': 'true' }, '?×');
  const go = h('button', { type: 'button', class: 'btn wide', disabled: true }, 'Lock it in');
  ...
  put($('final'), h('div', { class: 'card' }, dots(R()), h('p', { class: 'eb' }, 'Final guess'),
    h('p', { class: 'ask', tabindex: '-1' }, 'How many times its normal traffic did ', h('em', null, f.title), ' get?'),
    read, wrap, ticks, h('p', { class: 'coach' }, 'Tap or drag the track to guess. Within 1.5× scores 2, within 3× scores 1.'), go));
  $('final').scrollIntoView({ behavior: RM ? 'auto' : 'smooth', block: 'start' });
  $('final').querySelector('.ask').focus({ preventScroll: true });
}
function shot() {
  const f = finalInfo(), a = f.actual, g = S.guess, x = L.magX(g, a), pts = L.magPoints(g, a);
  const big = h('p', { class: 'big' }, '1.0×');
  let svg = null;
  if (f.opt && f.rr) {
  ...
  const el = h('div', { class: 'shot pop' },
    h('div', { class: 'strip' }, h('span', null, S.P.kind === 'practice' ? 'Knock-On practice' : `Knock-On #${S.n}`), h('span', { class: 'ashown' }, `#${S.n} · answer shown`)),
    h('p', { class: 't' }, `${f.emoji} ${f.title}`), big,
    h('p', { class: 'vs' }, `You said ${fmtM(g)}. It hit ${fmtM(a)}.`),
    ...
    h('p', { class: 'pts' }, pts ? `+${pts} point${pts > 1 ? 's' : ''} · within ${x.toFixed(1)}×` : `No points this time: ${x.toFixed(1)}× off (within 3× scores 1)`),
    h('p', { class: 'foot2' }, `${C.SITE} · Measured attention, not proof of cause.`));
  countUp(big, a, fmtM);
  return el;
}
async function lock(g) {
  S.guess = g;
  await S.rvP;
  put($('final'), h('div', { class: 'card', 'aria-live': 'polite' }, h('p', { class: 'eb' }, 'Final guess'), shot()));
  ...
  renderResult();
  renderCallit();
  setTimeout(() => $('result').scrollIntoView({ behavior: RM ? 'auto' : 'smooth', block: 'start' }), RM ? 0 : 1600);
...
async function restore(hh) {
  ...
  put($('round'), h('div', { class: 'card' }, h('p', { class: 'eb' }, `You played #${S.n}`), trail(R()), h('p', { class: 'stop' }, (S.rv && S.rv.end && S.rv.end.text) || '')));
...
function renderResult() {
  ...
  const items = [h('li', null, P.seed.emoji, h('b', null, P.seed.title), h('span', { class: 'm' }, fmtM(P.seed.multiple)))];
  c.forEach((r, k) => {
    if (!r.continues) items.push(h('li', { class: 'sep' }, `New ripple · ${r.seedEmoji} ${r.seedTitle}`));
    items.push(h('li', null, L.SQUARES[S.codes[k]], r.emoji, h('b', null, r.title), h('span', { class: 'm' }, fmtM(am(k).multiple)), h('span', { class: 'muted sm' }, L.fmtTiming(am(k).lag_days, am(k).timing))));
  });
  const share = h('button', { type: 'button', class: 'btn', onclick: () => doShare(text) }, 'Share');
  const copy = h('button', { type: 'button', class: 'btn ghost', onclick: () => copyText(text) }, 'Copy text');
  const next = L.nextRollover();
  put($('result'), h('div', { class: 'card' },
    h('p', { class: 'eb' }, 'Your chain'),
    h('p', { class: 'grid', 'aria-label': `Score ${score} of ${max}` }, L.gridEmoji(S.codes), h('span', { class: 'score' }, `${score}/${max}`)),
    rare ? h('p', { class: 'rare' }, rare, h('span', { class: 'tag', style: 'margin-left:6px' }, "players' picks")) : null,
    pl ? h('p', { class: 'sub' }, pl) : null,
    crowd ? h('p', { class: 'sub' }, crowd) : null,
    err ? h('p', { class: 'sub' }, err) : null,
    h('ol', { class: 'path' }, items),
    h('p', { class: 'sm muted' }, `📏 You guessed ${fmtM(S.guess)}; it hit ${fmtM(a)}. Measured attention, not proof of cause.`),
    h('div', { class: 'btns' }, share, copy),
    h('pre', { class: 'st', id: 'sharetext' }, text),
    h('p', { class: 'cd' }, 'Next chain in ', h('b', { 'data-cd': next }, L.fmtCountdown(next - Date.now())), live() && S.streak ? ` · 🔥 ${S.streak} in a row` : '')));
...
function callitTeaser() {
  if (S.P.callit && S.P.callit.options && S.P.callit.options.length) put($('callit'), h('div', { class: 'card' }, h('p', { class: 'eb' }, 'Call It'), h('p', { class: 'sub' }, 'Finish the chain to call which quiet page spikes next.')));
}
async function renderCallit() {
  ...
  const draw = split => put(list, ...co.options.map(o => {
    ...
    const out = d.outcome === 'hit' ? h('span', { class: 'out hit' }, mine === o.qid ? '🎯 called it' : '🎯 spiked') : d.outcome === 'miss' ? h('span', { class: 'out miss' }, mine === o.qid ? '✗ missed' : 'stayed calm') : null;
    const body = [h('span', { class: 'e', 'aria-hidden': 'true' }, o.emoji), h('span', { class: 'tx' }, h('b', null, o.title), h('small', null, o.desc), out, url ? [' ', h('a', { href: url, target: '_blank', rel: 'noopener', class: 'xs' }, 'Wikipedia ↗')] : null)];
    if (!shown) return h('button', { type: 'button', class: 'co', onclick: () => call(o.qid) }, body);
    return h('div', { class: 'co dis' + (mine === o.qid ? ' mine' : '') }, body,
      h('span', { class: 'st' }, h('strong', null, pct(d.model_p)), 'Model', cp != null ? [h('br'), `players ${cp}%`] : null));
  }));
  ...
  put($('callit'), h('div', { class: 'card' },
    ...
    h('p', { class: 'xs muted' }, `Window ${fmtD(co.window_start)} – ${fmtD(co.window_end)} · resolves ${fmtD(resolves)} · ${closed ? 'calls closed' : `calls close ${fmtD(L.isoDay(closes))} ${new Date(closes).toISOString().slice(11, 16)} UTC`}. A hit is any day at 3+ robust deviations and 1.5× normal.`),
    list, note, h('div', { class: 'rec', id: 'rec' })));
  record();
}
async function record() {
  ...
  if (!ns.length) return;
  const cs = await Promise.all(ns.map(getCallit)), el = $('rec');
  if (!el) return;
  let hit = 0, res = 0;
  const chips = ns.map((n, j) => {
    ...
    if (oc !== 'pending') res++;
    if (oc === 'hit') hit++;
    return h('span', { class: 'bdg' }, oc === 'hit' ? `🎯 #${n} called it` : oc === 'miss' ? `✗ #${n} missed` : `⏳ #${n} pending`);
  });
  put(el, h('span', { class: 'xs muted', style: 'width:100%' }, `Your calls: ${hit} of ${res} resolved called`), ...chips);
}

// ---------- tomorrow / price test ----------
function renderJoin(at) {
  const next = at || L.nextRollover(), form = h('form', { class: 'jf', hidden: true }), note = h('p', { class: 'sm' });
  let price = 'free';
  ...
  const chips = h('div', { class: 'chips wrapchips' }, Object.keys(L.CATS).map(c => h('button', { type: 'button', class: 'chip', 'aria-pressed': 'false', onclick: e => { const b = e.currentTarget, on = !topics.has(c); on ? topics.add(c) : topics.delete(c); b.setAttribute('aria-pressed', on); } }, `${L.CATS[c]} ${L.CAT_NAMES[c]}`)));
  const send = h('button', { class: 'btn wide', type: 'submit' }, 'Save my spot');
  form.append(note, h('label', { for: 'jem' }, 'Email'), email, h('label', { for: 'jrole' }, 'What do you do?'), role,
    h('fieldset', null, h('legend', null, 'Topics (optional)'), chips), send,
    h('p', { class: 'xs muted' }, 'We store your email, role and topics for this list only, plus a daily-salted hash of your IP for rate limits. Emails cannot be read back through the site. No spam: we write when something opens.'));
  form.addEventListener('submit', async e => {
    e.preventDefault();
    send.disabled = true;
    if (!FIX) await rpc('ripples_join', { p_email: email.value.trim(), p_role: role.value || null, p_price: price, p_topics: topics.size ? [...topics] : null, p_source: 'game' }, true);
    put(form, h('p', { class: 'sub', role: 'status' }, FIX ? 'Fixture mode: nothing was sent.' : price === 'free' ? "Saved. We'll write once, when the daily email starts." : "Saved. No charge. We'll email you once when Radar opens."));
  });
  const btns = [];
  const choose = (p, b) => {
    btns.forEach(x => x.setAttribute('aria-pressed', x === b));
    price = p;
    if (p === 'radar5' && C.STRIPE.radar5) { window.open(C.STRIPE.radar5, '_blank', 'noopener'); return; }
    note.textContent = p === 'free' ? "We don't send email yet. Leave your address and we'll write once, when the daily email starts." : "Not open yet. No charge. We'll email you once when it opens.";
    form.hidden = false; email.focus();
  };
  btns.push(h('button', { type: 'button', class: 'pb', 'aria-pressed': 'false', onclick: e => choose('free', e.currentTarget) }, h('b', null, "📬 Get tomorrow's chain"), h('small', null, 'Free daily email')),
    h('button', { type: 'button', class: 'pb', 'aria-pressed': 'false', onclick: e => choose('radar5', e.currentTarget) }, h('b', null, '📡 Founding Radar, $5/mo'), h('small', null, C.STRIPE.radar5 ? 'Email me when a trend starts spiking pages in my topics. Refundable, fulfilled by hand at first, cancel anytime.' : 'Email me when a trend starts spiking pages in my topics.')));
  put($('join'), h('div', { class: 'card' }, h('p', { class: 'eb' }, 'Tomorrow'), h('h2', null, "Get tomorrow's chain"),
    h('p', { class: 'sm muted' }, at ? 'Knock-On #1 goes live in ' : 'A new chain goes live every day at 07:30 UTC, the next in ', h('b', { 'data-cd': next }, L.fmtCountdown(next - Date.now())), '.'),
    h('div', { class: 'pt' }, btns), form));
}
...
async function board() {
  ...
  let cat = null;
  const W = 340, H = 230, ml = 26, mb = 30, mt = 8, mr = 8;
  ...
  [3, 10, 30, 100].forEach(v => svg.append(sv('text', { x: lx(v), y: H - mb + 12, 'text-anchor': 'middle' }, v + '×')));
  ...
  const drawList = () => put(list, ...T.filter(t => !cat || t.category === cat).sort((a, b) => b.splash_multiple - a.splash_multiple).map(t => {
    ...
    return h('li', null, h('span', { class: 'e', 'aria-hidden': 'true' }, t.emoji), h('b', null, t.title), h('span', { class: 'n' }, `${fmtM(t.splash_multiple)} · ${t.wake_k}/${t.wake_of}`),
      h('span', { class: 'q' }, t.sensitive || !t.quadrant ? 'Sensitive topic: no label' : QN[t.quadrant], t.biggest_in_days ? ` · biggest day in ${L.fmtInt(t.biggest_in_days)} days` : '',
        bf ? [' ', h('button', { type: 'button', class: 'lk', onclick: () => copyText(bf) }, 'Copy')] : null, crossRow(t.cross)));
  }));
  const chips = h('div', { class: 'chips', role: 'group', 'aria-label': 'Topic' });
  const mk = (c, label, n) => h('button', { type: 'button', class: 'chip', 'aria-pressed': String(c === cat), disabled: c && !n ? true : null, onclick: () => { cat = c; chips.querySelectorAll('.chip').forEach(b => b.setAttribute('aria-pressed', b.dataset.c === String(c))); pts.forEach((p, i) => p.setAttribute('opacity', !cat || T[i].category === cat ? 1 : 0.15)); drawList(); }, 'data-c': String(c) }, label);
  chips.append(mk(null, 'All', T.length), ...Object.keys(L.CATS).map(c => mk(c, `${L.CATS[c]} ${L.CAT_NAMES[c]}${cnt[c] ? ' ' + cnt[c] : ''}`, cnt[c] || 0)));
  drawList();
  put($('board'), h('div', { class: 'card' }, h('p', { class: 'eb' }, `Today's Board · ${fmtD(B.date)}`), h('h2', null, 'Where attention went today, by topic'),
    h('p', { class: 'sm muted' }, 'Every trend measured today. Splash: its biggest day against its own normal readers. Wake: how many of a fixed set of 20 linked pages spiked after it.'),
    chips, svg, h('p', { class: 'cap' }, 'Wake k of 20 on the vertical axis. Measured attention, not proof of cause.'), list));
}

// ---------- practice: which spiked harder? ----------
function practice() {
  const out = h('div'), btn = h('button', { type: 'button', class: 'btn ghost wide', onclick: () => startPractice(out, btn) }, 'Play practice');
  put($('practice'), h('div', { class: 'card' }, h('p', { class: 'eb' }, 'Practice'), h('h2', null, 'Which spiked harder?'),
    h('p', { class: 'sm muted' }, 'Two measured hops from past puzzles. Pick the one that ran further above its own normal readers. Scored on this device only.'), btn, out));
}
async function startPractice(out, btn) {
  btn.hidden = true;
  put(out, h('p', { class: 'sm muted' }, 'Loading past hops…'));
  let arc = FIX ? await fx('archive-fixture.json') : await load('archive.json', 'ripples_archive', { p_limit: 500, p_kind: 'all' });
  arc = (arc || []).filter(e => e.n !== S.n || S.guess);
  ...
  if (pool.length < 2) { put(out, h('p', { class: 'sm muted' }, 'Practice opens once past puzzles are archived.')); return; }
  let streak = 0;
  ...
  const round = () => {
    ...
    if (a === b) { put(out, h('p', { class: 'sm muted' }, 'Not enough distinct hops yet.')); return; }
    const say = h('p', { class: 'say', 'aria-live': 'polite' }), top = sharedTop([a.m, b.m]);
    const cards = [a, b].map(x => h('button', { type: 'button', class: 'opt', onclick: () => choose(x) }, h('span', { class: 'e', 'aria-hidden': 'true' }, x.emoji), h('b', null, x.title), h('small', null, `linked from ${x.parent}`)));
    const choose = x => {
      ...
      [a, b].forEach((y, i) => { const c = cards[i]; c.disabled = true; c.classList.add('done', y.m.multiple >= (y === a ? b : a).m.multiple ? 'yes' : 'no');
        c.querySelector('small').replaceWith(h('div', { class: 'rv' }, spark(ratio(y.m), { h: 38, top, med: 1, line: 'draw' })), h('span', { class: 'rl' }, h('strong', null, fmtM(y.m.multiple) + ' normal'), ` · ${L.fmtTiming(y.m.lag_days, y.m.timing)} ${y.parent}`)); });
      say.textContent = `${win ? '✓ Right.' : '✗ Not this time.'} Streak ${streak} · best ${Math.max(best(), streak)}`;
      say.after(h('button', { type: 'button', class: 'btn ghost wide', style: 'margin-top:10px', onclick: round }, 'Next pair →'));
    };
