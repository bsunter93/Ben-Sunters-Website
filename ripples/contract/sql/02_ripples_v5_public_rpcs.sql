...
-- Hash-chained ledger row (idempotent). Live puzzles only are chained by callers.
create or replace function ripples._ledger_write(p_n int) returns text
language plpgsql volatile security definer set search_path = '' as $$
declare r ripples.puzzles; cl jsonb; ph text; prev text; ch text;
begin
  select chain_hash into ch from ripples.ledger where n = p_n;
  if ch is not null then return ch; end if;
  select * into r from ripples.puzzles where n = p_n;
  if not found then return null; end if;
  select coalesce(jsonb_agg(jsonb_build_array(c.qid, c.model_p) order by c.qid), '[]'::jsonb)
    into cl from ripples.callit c where c.n = p_n;
  ph := encode(extensions.digest(
          jsonb_build_object('n', p_n, 'payload', r.payload, 'reveal', r.reveal, 'callit', cl)::text, 'sha256'), 'hex');
  prev := coalesce((select l.chain_hash from ripples.ledger l where l.n < p_n order by l.n desc limit 1), repeat('0', 64));
  ...
  return (select chain_hash from ripples.ledger where n = p_n);
end $$;
...
begin
  ...
  if r.kind = 'live' and (now() at time zone 'utc')::date > c.window_start + 1 then raise exception 'closed'; end if;
  iph := ripples._hash(ripples._ip());
...
begin
  ...
  on conflict (email_norm) do update
     set role   = coalesce(excluded.role, ripples.waitlist.role),
         price  = coalesce(excluded.price, ripples.waitlist.price),
...
begin
  ...
  return jsonb_build_object(
    ...
    'past', r.puzzle_date < (now() at time zone 'utc')::date,
...
begin
  ...
  if nn is not null then
    ...
    if r.kind = 'live' then ledger_head := ripples._ledger_write(nn); end if;
    select * into r from ripples.puzzles where n = nn;
  end if;
