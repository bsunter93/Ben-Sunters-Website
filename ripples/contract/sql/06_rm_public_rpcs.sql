...
begin
  ...
  if sc is not null then
    ...
    asserts := asserts || jsonb_build_object('check', 'the leak guard catches source keys (label, arrow path, sentence) and passes names', 'ok',
                 ...
                 and cardinality(ripples.rm_label_leaks('{"nodes":[{"node":"poly.mkt:e:1061358","label":"e:1061358","path":[{"text":"Xi Jinping → e:1061358"}],"sentence":"e:1061358 ran 2.00× its normal"},{"node":"fred:DGS10","label":"DGS10"},{"node":"iem.warn:__total__","label":"Storm → __total__"}]}'::jsonb)) = 5
